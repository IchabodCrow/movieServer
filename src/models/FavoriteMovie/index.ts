...
export default {
  Mutation: {
    addMovie: async (args, { id, movieId }) => {
      ...
      let favoriteMovieFromDB = await getRepository(FavoriteMovies).findOne({
        movieId: movieId,
      });
      if (!favoriteMovieFromDB) {
        favoriteMovieFromDB = await getRepository(FavoriteMovies).create({
          movieId: movieId,
    ...
    deleteMovie: async (args, { movieId }) => {
      ...
      let favoriteMovieFromDBRemove = await favoriteMovieFromDB.find({
        movieId: movieId,
        userId: userInSystem.id,
      });
